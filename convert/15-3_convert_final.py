...
def process_label_data(label_data):
    ...
    for item in label_data:
        ...
        for annotation in annotations:
            class_num += 1  # 각 annotation별로 class_num 증가
            class_name = annotation.get('name', "")
            
            # '데이터 종류_noChange'는 필터링하여 제외
            if class_name == "데이터 종류_noChange":
                continue  # 건너뜀

            values = annotation.get('value', [])
            class_info_list = process_annotation(values)
        ...
        # 각 라벨 항목을 딕셔너리에 저장 (데이터 종류_noChange가 필터링된 상태로 저장됨)
        label_dict[data_id] = {
...
def generate_final_json(label_file, refine_file):
    ...
    # 최종 데이터를 JSON 파일로 저장
    with open('15-3_output_data.json', 'w', encoding='utf-8') as f_final:
        json.dump(final_data, f_final, indent=4, ensure_ascii=False)

    print("Final JSON file created successfully.")

# 호출하여 파일을 처리
label_file_path = 'C:\\Users\\admin\\Desktop\\syntax_check_converter\\2024-nia15-3-label-new.json'
refine_file_path = 'C:\\Users\\admin\\Desktop\\syntax_check_converter\\2024-nia15-3-refine-new.json'

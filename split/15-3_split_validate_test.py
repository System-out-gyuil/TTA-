...
# 텍스트 이미지 분리하기
def problem_type_split(problem_type, path):
    if problem_type == '주관식':
        path += '01.주관식\\'
    else :
        path += '02.객관식\\'
...
for data in tqdm(datas):
    ...
    # 파일 저장할 경로
    path = 'C:\\Users\\admin\\Desktop\\syntax_check_converter - 복사본\\test_gyuil\\15-3_split_test\\'

    # 제목
    title = data.get('source_data_info').get('source_data_name').replace("공통학년", "3")
    ...
    raw_data_info.get('grade').replace("공통학년", "3학년")


    source_data_info = data.get('source_data_info')

    source_name = source_data_info.get('source_data_name').replace("공통학년", "3")
    ...
    problem_type = source_data_info.get('types_of_problems')
    ...
    if not learning_data_info == []:
        ...
        class_info_list = learning_data_info[0].get('class_info_list')

        if not class_info_list == []:
            description = class_info_list[0].get('text_description')
        ...
        # 주관식 객관식 나누기
        path = problem_type_split(problem_type, path)
        ...
        # 유효성 검사 통과 못한 데이터들
            ...
            path = 'C:\\Users\\admin\\Desktop\\syntax_check_converter - 복사본\\test_gyuil\\15-3_split_test\\99.비정상 데이터\\'
            makedirs(path)
